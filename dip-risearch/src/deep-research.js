...
class CustomSearchAPI {
    baseUrl;
    constructor(baseUrl = 'http://34.30.168.182') {
...
const customSearchAPI = new CustomSearchAPI(process.env.CUSTOM_API_BASE_URL || 'http://34.30.168.182');
    ...
    const res = await generateObject({
        ...
        schema: z.object({
            queries: z
                ...
                researchGoal: z
                    .string()
                    .describe('First talk about the goal of the research that this query is meant to accomplish, then go deeper into how to advance the research once the results are found, mention additional research directions. Be as specific as possible, especially for additional research directions.'),
            }))
...
async function processSerpResult({ query, result, numLearnings = 4, numFollowUpQuestions = 4, }) {
    ...
    const contents = compact(result.results.map(item => {
    ...
    if (contents.length === 0 && result.results.length > 0) {
        ...
        const fallbackContents = compact(result.results.map(item => {
...
export async function writeFinalAnswer({ prompt, learnings, }) {
    const learningsString = learnings
        .map(learning => `<learning>\n${learning}\n</learning>`)
        .join('\n');
    const res = await generateObject({
...
export async function deepResearch({ query, breadth, depth, learnings = [], visitedUrls = [], onProgress, }) {
    ...
    const results = await Promise.all(serpQueries.map(serpQuery => limit(async () => {
        try {
